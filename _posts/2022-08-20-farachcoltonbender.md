...
이렇게 표현될 수 있는 서로 다른 배열의 개수는 $2^{K-1}$개 이고,  $2^{K-1} = 2^{0.5 \log(N)-1} = 0.5(2^{\log(N)})^{0.5} = 0.5 \sqrt N = O(\sqrt N)$입니다. 만들어질 수 있는 $O(\sqrt N)$개의 배열을 모두 비트마스크를 이용해 하나의 수로 나타내어, $\text{block}[\text{mask}][l][r]$ 배열을 채워준다면 $O(\sqrt{N} K^2) = O(\sqrt{N} \log^2(N)) = O(N)$ 에 만들어질 수 있는 모든 블럭 내부의 RMQ를 미리 계산해줄 수 있습니다.
